<template>
    <div class="colddrink">
        <h1><i @click="backMenu" class="glyphicon glyphicon-arrow-left special"></i>精品点心</h1>
        <Lunbo></Lunbo>
        <div  class="cmes">
        <ul class="coldUl">
          <li class="coldLi" v-for="(item,index) in dataset">
          <img :src="item.imgurl"><div class="spe coldDiv5"><span>{{item.name}}</span><p class="speP" @click="buy($event,item.id)"><span>{{item.price}}</span><el-input-number class="coldInput" v-model="'num'+item.id" :min="0" :max="20" label="0" lazy></el-input-number></p></div>
          </li>
        </ul>
        </div>
        <bottomnav></bottomnav>
...
<script>
    ...
    import Lunbo  from  './carouselDianxin.vue'
